...
def get_weather_aggregator(selected_weather_API_class_names: list[str], region: str) -> WeatherAggregator:
    if not selected_weather_API_class_names:
        raise HTTPException(status_code=404, detail="Enter at least one API")

    selected_weather_API_classes = tuple([weather_API_class for weather_API_class in weather_API_classes if weather_API_class.__name__ in selected_weather_API_class_names])

    return CachedWeatherAggregator.get_weather_aggregator(selected_weather_API_classes, region)
...
def get_hour_forecast(region: str, day: int, hour: int, API_class_names: list[str] = Query(None)):
    weather_aggregator = get_weather_aggregator(API_class_names, region)
